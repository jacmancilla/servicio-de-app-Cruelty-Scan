CREATE TABLE `crueltyscan`.`comuna` (
  `id_comuna` INT NOT NULL,
  `nom_comuna` VARCHAR(45) NOT NULL,
  `id_region` INT NOT NULL,
  PRIMARY KEY (`id_comuna`));

  ALTER TABLE `crueltyscan`.`comuna` 
ADD INDEX `fk_comuna_region_id_idx` (`id_region` ASC);
;
ALTER TABLE `crueltyscan`.`comuna` 
ADD CONSTRAINT `fk_comuna_region_id`
  FOREIGN KEY (`id_region`)
  REFERENCES `crueltyscan`.`region` (`id_region`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;

INSERT INTO `comuna` (`id_comuna`, `nom_comuna`, `id_region`)
VALUES
  (1,'Arica',1),
  (2,'Camarones',1),
  (3,'General Lagos',1),
